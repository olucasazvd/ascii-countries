...
find "pngs" -type f -name "*.svg" -exec sh -c 'inkscape -w 1024 -h 1024 "$1" -o "${1%.svg}.png"' _ {} \;
# Rename the PNG files to the name of the directory they are in
find "pngs" -type f -name "*.png" -exec sh -c 'mv "$1" "$(dirname "$1")/$(basename "$(dirname "$1")").png"' _ {} \;
# Remove the original SVG files
find pngs/ -type f -name "*.svg" -exec rm -f {} \;
